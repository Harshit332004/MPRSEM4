...
def load_db_info(file_path="db.dat"):
    try:
...
def sqlcreate():
    # Replace these values with your actual database username and password
    actual_username = "root"
    actual_password = "332004"

    # Load other database connection information from "db.dat"
    db_info = load_db_info()

    if db_info:
        db_hs = db_info.get("host", "localhost")
        db_us = actual_username  # Use the actual username here
        db_pw = actual_password  # Use the actual password here
        db_db = db_info.get("db", "movie_ticketbooking")

        try:
            # Explicitly provide the password in the connection parameters
            dataBase = mysql.connector.connect(
                host=db_hs, user=db_us, password=db_pw, database=db_db
            )
        except mysql.connector.Error as err:
            dataBase = mysql.connector.connect(host=db_hs, user=db_us, password=db_pw)
            mc = dataBase.cursor()
            mc.execute(f"CREATE DATABASE IF NOT EXISTS {db_db};")
            mc.execute(f"USE {db_db};")
            mc.execute(
                "CREATE TABLE IF NOT EXISTS acc (username VARCHAR(255) PRIMARY KEY, phno VARCHAR(255), pass VARCHAR(255)) ENGINE=InnoDB DEFAULT CHARSET=latin1"
            )
            mc.execute("DESC acc;")

# Call the sqlcreate function
sqlcreate()
...
def w():
    ...
    lb = Button(
        ...
        pady=4,
        relief="groove",
        command=lambda: checkpass(entry1, entry2, pp, db_hs, db_us, db_pw, db_db),
    )
    lb.place(x=140, y=210)
...
def checkpass(entry1, entry2, pp, db_hs, db_us, db_pw, db_db):
    a = entry1.get().strip()
    b = entry2.get().strip()
    ...
    try:
        ...
        if len(a) == 0:
        ...
        elif myresult:  # Check if there is any result
            username = entry1.get()
            password = entry2.get()
            pp.config(
            ...
            # Assuming that ro and main are globally accessible
            ro.destroy()
...
pp = Label(ro, text="", bg="black", fg="white", font=("Helvetica", 8))
pp.place(x=100, y=180)
...
# checkpass(entry1, entry2, pp, "your_host", "your_user", "your_password", "your_database")
